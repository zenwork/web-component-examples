(function () {
    ...
    /*
     * logging interface that fires to visual console if available
     */
    const console = {
    ...
    /*
     * bootstrap component using x-tag
     */
    xtag.register('rps-xtag', {
        /* when used with shadow dom */
        shadow: game.template,
        /* when used without shadow dom */
        content: game.template,
        lifecycle:{
            created:function () {
                console.log('rps-xtag', 'lifecycle', 'created');
                if (this.shadowRoot) {
                    this.shadowRoot.game = game.createGame();
                } else {
                    this.game = game.createGame();
                }
            },
            inserted:function () {
                console.log('rps-xtag', 'lifecycle', 'inserted');
                if (this.shadowRoot) {
                    game.wireUp(this.shadowRoot.children[0], this.shadowRoot.game);
                } else {
                    game.wireUp(this.children[0], this.game);
                }
            }
        },
        accessors:{
            'player-1':{
                ...
                set:function (value) {
                    ...
                    if (this.shadowRoot) {
                        game.updateName(value, this.shadowRoot.game, this.shadowRoot.children[0]);
                    } else {
                        game.updateName(value, this.game, this.children[0]);
                    }
                }
            }
        }
